...
try {
    ...
    // Consulta SQL para obtener los datos de la tabla 'cita' con sus datos relacionados de 'paciente' y 'doctor'
    $sql = "SELECT c.idCitas, CONCAT(p.nombre, ' ', p.apellidoPaterno) AS nombrePaciente, 
    CONCAT(d.nombre, ' ', d.apellidoPaterno) AS nombreDoctor, 
    DATE_FORMAT(c.fechaHora, '%d/%m/%Y %H:%i') AS fechaHoraFormateada, c.estado AS estado
FROM cita c
...
ORDER BY CASE c.estado
    ...
    $resultado = $conexion->query($sql);
      ...
      echo "<div class='header-item'>Paciente</div>";
      ...
      while ($fila = $resultado->fetch_assoc()) {
          ...
          echo "<div class='table-data'>" . $fila['nombrePaciente'] . " " . $fila['apellidoPaciente'] . "</div>";
          echo "<div class='table-data'>" . $fila['nombreDoctor'] . " " . $fila['apellidoDoctor'] . "</div>";
          echo "<div class='table-data'>" . $fila['fechaHoraFormateada'] . "</div>";
          echo "<div class='table-data'>" . $fila['estado'] . "</div>";
          echo "<div class='table-data'>
                  <button onclick=\"editarCita(" . $fila['idCitas'] . ")\">Editar</button>
